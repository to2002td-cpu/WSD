...
def sense_curves(table, word, pos, out_path, metric="excess_k10", layer=16):
    """Separation over pretraining for each sense of one lemma, coloured by how
    frequent the sense is. Makes the within-lemma relation visible directly."""
    import matplotlib.colors as colours

    data = table[(table["word"] == word) & (table["pos"] == pos) & (table["layer"] == layer)]
    if data.empty:
        raise SystemExit("no rows for " + word + " (" + pos + ") at layer " + str(layer))

    shares = data.groupby("sense")["share"].first().sort_values(ascending=False)
    norm = colours.LogNorm(vmin=max(shares.min(), 1e-4), vmax=shares.max())
    palette = plt.get_cmap("viridis")

    fig, ax = plt.subplots(figsize=(7.5, 4.8))
    for sense, share in shares.items():
        series = data[data["sense"] == sense].sort_values("step")
        steps = series["step"].to_numpy()
        steps = np.where(steps == 0, 1, steps)
        ax.plot(steps, series[metric].to_numpy(), marker="o", markersize=3,
                color=palette(norm(share)),
                label=sense + "  " + str(round(100 * share, 1)) + "%")
    ax.set_xscale("log")
    ax.set_xlabel("training step")
    ax.set_ylabel(metric)
    ax.set_title(word + " (" + pos + "), layer " + str(layer) + ", senses by corpus share")
